...
impl ScanRegistry {
    ...
    pub fn record_scan(&mut self, wallet: String, risk_score: u8, category: String, evidence_hash: String, summary: String) {
        assert!(risk_score <= 100, "risk score must be between 0 and 100");
        let caller = self.env().caller();
        let now = self.env().get_block_time().millis();
...
mod tests {
    ...
    fn records_scan() {
        ...
        let record = contract.get_scan("account-hash-demo".to_string()).unwrap();
